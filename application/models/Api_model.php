...
class Api_model extends CI_Model {
    ...
    function get_all_video_by_movie_id($videos_id){
        ...
        if($query->num_rows() > 0):
            ...
            foreach ($videos as $video):
                ...
                $response[$i]['label']                          = $video['file_source'];//'Server#'.($i  +1);
                $response[$i]['stream_key']                     = $video['stream_key'];
                $response[$i]['file_type']                      = $video['file_source'];
                $response[$i]['file_url']                       = $video['file_url'];
                if($video['file_source'] =='youtube'):
                    $response[$i]['file_type']                      = 'embed';
                    $response[$i]['file_url']     = str_replace('watch?v=','embed/',$video['file_url']);
                elseif($video['file_source'] =='gdrive'):
                    $response[$i]['file_type']          = 'embed';
    ...
    *****/
    ...
    function get_related_tvseries($videos_id='',$genre_ids=''){
        ...
        $this->db->where('is_tvseries','1');
        $this->db->where('publication','1');
        //$this->db->like('title',$this->common_model->get_title_by_videos_id($videos_id),'both');
        //$this->db->like('genre',$genre_id,'both');
        $this->db->limit(6);
    ...
    *****/
    ...
    public function get_single_tv_details_by_id($id=''){
        ...
        $response['stream_label']   = $tv->stream_label;
    ...
    function get_aditional_media_source($live_tv_id=''){
        ...
        if($query->num_rows() > 0):
            ...
            foreach ($live_tv_urls as $live_tv_url):
                ...
                $response[$i]['label']              = $live_tv_url['label'];
                $response[$i]['url']                = $live_tv_url['url'];
                if($live_tv_url['source'] =='youtube'):
                    $response[$i]['url']     = str_replace('watch?v=','embed/',$tv->stream_url);
                endif;
                $i++;
            endforeach;
        endif;
    ...
    **********************/
    ...
    public function get_search_result($q='',$page=''){
        $response = array();
        if(!empty($page) && $page !='' && $page !=NULL && is_numeric($page)):
            $offset = ((int)$page *   $this->default_limit)   -   $this->default_limit;
            $this->db->limit($this->default_limit,$offset);
