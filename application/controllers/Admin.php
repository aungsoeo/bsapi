...
class Admin extends CI_Controller {   
    ...
    function send_push_notification($param1 = '', $param2 = ''){
        if ($this->session->userdata('admin_is_login') != 1)
            redirect(base_url(), 'refresh');
            // active menu session
            $this->session->unset_userdata('active_menu');
        ...
        $data['page_name']      = 'send_push_notification';
        $data['page_title']     = 'Send Push Notification(OneSignal)';
        $this->load->view('admin/index', $data);
    }
        ...
        if(!empty($videos_id) && $videos_id !='' && $videos_id !=NULL && is_numeric($videos_id)):
            ...
            if($verify):
                if($type == 'email'):
                    ...
                    $this->session->set_flashdata('success', 'Newslater Send Successfully.');
                else:
                    $this->load->model('notify_model');
        ...
        if(isset($_FILES["FileInput"]) && $_FILES["FileInput"]["error"]== UPLOAD_ERR_OK)
            {
                ...
                if(move_uploaded_file($_FILES['FileInput']['tmp_name'], $UploadDirectory.$NewFileName ))
                {
                    ...
                    $response['status'] = 'success';
                    $response['video_info'] = '<tr id="row_'.$episodes_id.'"><td><strong>'.$episodes_name.'</strong><td><strong>Upload</strong></td><td>'.base_url().'uploads/videos/'.$NewFileName.'</a></td><td><a href="#" onclick="javascript:location.reload();">Reload Page</a></td><td><a  href="#"onclick="javascript:location.reload();">Reload Page</a></td></tr>';
                }else{
                    $response['status'] = 'error';
                    $response['errors'] = 'error uploading File!';
                }                
